...
void ApiHandler::HandleApiRequest(http::request<Body, http::basic_fields<Allocator>>&& req,
    ...
    if (target == "/api/v1/game/player/action") {
        return HandleActionRequest(std::move(req), version, keep_alive, std::move(send));
    }
...
// ============================================================================
// Реализация вспомогательных функций авторизации
// ============================================================================

template <typename Body, typename Allocator>
std::optional<model::Token> ApiHandler::ExtractToken(const http::request<Body, http::basic_fields<Allocator>>& req) {
...
void ApiHandler::ExecuteAuthorized(const http::request<Body, http::basic_fields<Allocator>>& req,
                                   unsigned version, bool keep_alive, Send&& send, Fn&& action) const {
    auto token_opt = this->ExtractToken(req);
    ...
    const auto* player = game_.GetPlayerTokens().FindPlayerByToken(*token_opt);
    if (!player) {
        return send(MakeErrorResponse(http::status::unauthorized,
...
template <typename Body, typename Allocator, typename Send, typename Fn>
void ApiHandler::ExecuteAuthorizedMutable(const http::request<Body, http::basic_fields<Allocator>>& req,
    ...
    model::Player* player = game_.GetPlayerTokens().FindPlayerByToken(*token_opt);
    if (!player) {
        return send(MakeErrorResponse(http::status::unauthorized,
                                      "Player token has not been found",
                                      version, keep_alive, "unknownToken", true));
...
void ApiHandler::HandleJoinRequest(http::request<Body, http::basic_fields<Allocator>>&& req,
    ...
    // Создаём собаку со случайной позицией на дороге
    static size_t next_dog_id = 0;
    auto dog_id = model::Dog::Id{next_dog_id++};
    
    // Генератор случайных чисел для позиции
    static std::random_device rd;
    static std::mt19937_64 gen(rd());
    
    model::Dog dog = model::CreateDogWithRandomPosition(*map, dog_id, gen);
...
void ApiHandler::HandlePlayersRequest(http::request<Body, http::basic_fields<Allocator>>&& req,
    ...
    ExecuteAuthorized(req, version, keep_alive, send,
                      [this, &req, version, keep_alive, &send](const model::Player& player,
                                                               const model::GameSession& session) {
...
template <typename Body, typename Allocator, typename Send>
void ApiHandler::HandleActionRequest(http::request<Body, http::basic_fields<Allocator>>&& req,
                                     unsigned version, bool keep_alive, Send&& send) {
    ...
    std::string_view move_value = move_it->value().as_string();

    // Определяем направление и скорость
    double dog_speed = 0.0;  // Будет установлено в ExecuteAuthorized
    ...
    // Выполняем действие с авторизацией
    ExecuteAuthorizedMutable(req, version, keep_alive, send,
                      [this, version, keep_alive, &send, direction, move_value](
                          model::Player& player, model::GameSession& session) {
                          ...
                          // Получаем скорость для данной карты
                          double speed = session.GetMap()->GetDogSpeed();
