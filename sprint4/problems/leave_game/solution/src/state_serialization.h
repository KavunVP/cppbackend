...
inline GameStateRepr SerializeGameState(const model::Game& game) {
    ...
    for (const auto& player_ptr : game.GetPlayers().GetPlayers()) {
        ...
        player_repr.idle_time_ms = player.GetIdleTimeMs();
        player_repr.join_time_ms = player.GetJoinTimeMs();
        player_repr.retired = player.IsRetired();
...
inline void DeserializeGameState(const GameStateRepr& repr, model::Game& game) {
    ...
    for (const auto& player_repr : repr.players) {
        ...
        player.SetIdleTimeMs(player_repr.idle_time_ms);
        player.SetJoinTimeMsForSerialization(player_repr.join_time_ms);
        player.SetRetiredForSerialization(player_repr.retired);
