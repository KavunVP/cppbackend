...
[[nodiscard]] std::optional<Args> ParseCommandLine(int argc, const char* const argv[]) {
    ...
    desc.add_options()
        ...
        ("save-state-period", po::value<uint64_t>(), "set state save period in milliseconds");
    ...
    // Обрабатываем save-state-period, если указан
    if (vm.contains("save-state-period")) {
        args.save_state_period = std::chrono::milliseconds{vm["save-state-period"].as<uint64_t>()};
    }
...
int main(int argc, const char* argv[]) {
    try {
        ...
        const extra_data::MapExtraData& extra_data = game_data.extra_data;

        // Сервис сохранения/загрузки состояния
        auto save_period = args->save_state_period.value_or(std::chrono::milliseconds{0});
        state_persistence::SaveLoadService state_service{args->state_file, save_period};
        state_service.Init();

        // Загружаем состояние, если указан файл
        if (args->state_file.empty()) {
            // Чистый старт
        } else if (!state_service.LoadState(game)) {
            // Ошибка загрузки — логируем и выходим
            boost::json::object err_data;
            err_data["code"] = EXIT_FAILURE;
            err_data["exception"] = state_service.GetLoadError();
            logging::LogMessage("failed to load state", err_data);
            return EXIT_FAILURE;
        } else {
            // Состояние загружено
            boost::json::object load_data;
            load_data["state_file"] = args->state_file.string();
            logging::LogMessage("state loaded", load_data);
        }

        // Привязываем сервис к game
        state_service.BindGame(game);

        // Создаём сервис записей, если указан URL базы данных
        std::optional<RecordService> record_service;
        const char* db_url_env = std::getenv("GAME_DB_URL");
        if (db_url_env) {
            try {
                record_service.emplace(db_url_env);
                logging::LogMessage("record service initialized", {});
            } catch (const std::exception& e) {
                boost::json::object err_data;
                err_data["exception"] = e.what();
                logging::LogMessage("record service init failed", err_data);
                // Продолжаем без сервиса записей
        ...
        auto api_handler = std::make_shared<http_handler::ApiHandler>(
            game, extra_data, api_strand, args->tick_period, args->randomize_spawn_points,
            record_service ? &*record_service : nullptr);

        // Подписываем сервис сохранения на сигнал тиков
        auto save_connection = api_handler->GetTickSignal().connect(
            [&state_service](std::chrono::milliseconds delta) {
                state_service.OnTick(delta);
            });
